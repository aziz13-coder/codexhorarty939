...
class EnhancedTraditionalHoraryJudgmentEngine:
    ...
    def _classify_question_intent(self, question: str, category: str) -> str:
        ...
        safety_indicators = ('safe', 'safety', 'is she safe', 'is he safe', 'are they safe')
    ...
    def _apply_enhanced_judgment(self, chart: HoraryChart, question_analysis: Dict,
        ...
        if perfection["perfects"]:
            ...
            if perfection["type"] == "direct_penalized":
                ...
                if "sextile" in aspect_name or "trine" in aspect_name:
                    rule_text = "Perfection with obstacles"
                else:
                    rule_text = "Perfection by hard aspect - YES, but with obstacles"
                reasoning.append({"stage": "Perfection found", "rule": rule_text, "weight": 4})
    ...
    def _void_traditional_ground_truth(self, chart: HoraryChart) -> Dict[str, Any]:
        ...
        # LOG: Start VOC determination
        logging.info(f"[VOC] Starting void of course determination for Moon")
        logging.info(f"[VOC] Moon position: {moon_pos.longitude:.2f}° in {moon_pos.sign.sign_name if hasattr(moon_pos.sign, 'sign_name') else str(moon_pos.sign)}")
        logging.info(f"[VOC] Moon speed: {moon_pos.speed:.4f}°/day, retrograde: {moon_pos.retrograde}")
        ...
        logging.info(f"[VOC] Moon degree in sign: {moon_degree_in_sign:.2f}°")
        logging.info(f"[VOC] Degrees to sign exit: {degrees_to_sign_exit:.2f}°")
        ...
        if days_until_sign_exit is None:
            ...
            logging.info(f"[VOC] Using fallback speed calculation: {speed:.2f}°/day")
        else:
            logging.info(f"[VOC] Using precise exit timing from helper")
            
        logging.info(f"[VOC] Days until sign exit: {days_until_sign_exit:.4f}")

        # Check if Moon is stationary (cannot be void if not moving)
        if abs(moon_pos.speed) < config.timing.stationary_speed_threshold:
            logging.info(f"[VOC] Moon is stationary (speed {abs(moon_pos.speed):.4f} < threshold {config.timing.stationary_speed_threshold}) - not void")
            return {
                "void": False,
                "exception": False,
        ...
        logging.info("[VOC] Using enhanced aspects: calculate_moon_next_aspect(ignore_orb_for_voc=True)")
        moon_next_aspect = calculate_moon_next_aspect(
        ...
        if moon_next_aspect:
            logging.info(f"[VOC] Found applying aspect: {moon_next_aspect.aspect.display_name} to {moon_next_aspect.planet.value} in {eta:.4f} days")
        else:
            logging.info(f"[VOC] No applying aspects found within current sign")
        ...
        if moon_next_aspect is None or eta is None:
            ...
            logging.info(f"[VOC] DECISION: Moon is VOID OF COURSE")
            logging.info(f"[VOC] Sign mitigation: {sign_mitigation} (Moon in {moon_pos.sign.sign_name})")
        ...
        logging.info(f"[VOC] DECISION: Moon is NOT void of course - will {moon_next_aspect.aspect.display_name.lower()} {moon_next_aspect.planet.value}")
        return {
            "void": False,
            "exception": False,
            "reason": (
    ...
    def _debug_compare_moon_aspect_systems(self, chart: HoraryChart):
        ...
        logging.info("[ASPECT-DEBUG] ===== COMPARING MOON ASPECT SYSTEMS =====")
        ...
        logging.info(f"[ASPECT-DEBUG] Moon at {moon_pos.longitude:.2f}° in {moon_pos.sign.sign_name}")
        logging.info(f"[ASPECT-DEBUG] Moon speed: {moon_pos.speed:.4f}°/day, retrograde: {moon_pos.retrograde}")
        ...
        logging.info(f"[ASPECT-DEBUG] === SYSTEM 1: Enhanced Aspects (Moon-only) ===")
        if moon_enhanced:
            ...
            logging.info(f"[ASPECT-DEBUG] No Moon aspects found in enhanced list")

        # 2) VOC-mode (enhanced Moon-next, ignore orb)
        logging.info(f"[ASPECT-DEBUG] === SYSTEM 2: VOC Mode (ignore_orb_for_voc=True) ===")
        ...
        if moon_next_voc:
            ...
            logging.info(f"[ASPECT-DEBUG] No next aspect found (VOC mode)")
        
        # 4) Detailed comparison
        logging.info(f"[ASPECT-DEBUG] === DETAILED PLANET-BY-PLANET COMPARISON ===")

        for planet in classical_planets:
            if planet not in chart.planets:
                continue
            planet_pos = chart.planets[planet]
            logging.info(f"[ASPECT-DEBUG] --- {planet.value} at {planet_pos.longitude:.2f}° ---")
        ...
        try:
            ...
            if moon_next_voc and enhanced_next:
                ...
                logging.info(f"[ASPECT-DEBUG] SUMMARY: Enhanced vs VOC => {status}")
            elif not moon_next_voc:
                logging.info("[ASPECT-DEBUG] SUMMARY: VOC reports Moon void (no next aspect within sign).")
            else:
                logging.info("[ASPECT-DEBUG] SUMMARY: Enhanced has no Moon applying within sign; VOC found one.")
        except Exception as e:
            logging.warning(f"[ASPECT-DEBUG] SUMMARY step failed: {e}")
        
        logging.info("[ASPECT-DEBUG] ===== END MOON ASPECT SYSTEMS COMPARISON =====")
        ...
        for aspect in chart.aspects:
            if aspect.planet1 == planet:
                ...
                if target_planet in malefics:
                    # Only hard aspects can prohibit; ignore benefic trines/sextiles to malefics
                    if aspect.aspect not in {Aspect.CONJUNCTION, Aspect.SQUARE, Aspect.OPPOSITION}:
                        continue
                    
                    # Prefer timing over raw 'applying' flag; skip separating/past contacts
                    aspect_timing = getattr(aspect, 'time_to_perfection', None)
                    if aspect_timing is not None:
                        if aspect_timing <= 0:
                            # past / separating → not a prohibition
                            continue
                    else:
                        # No timing: fall back to applying flag
                        if not getattr(aspect, 'applying', False):
                            continue
                    ...
                    # Determine kind relative to any upcoming perfection between significators
                    has_upcoming_sig_perfection = (earliest_perfection_days is not None)
                    if has_upcoming_sig_perfection and aspect_timing is not None and aspect_timing < earliest_perfection_days:
                        # True prohibition: preempts the significators' perfection
                    ...
                    elif has_upcoming_sig_perfection and aspect_timing is not None and aspect_timing > earliest_perfection_days:
                        # Happens after perfection → corruption/complication, not prohibition
                        ...
                        # No upcoming sig perfection to prohibit → classify as affliction
                        kind = "affliction"
                        weight = self._calculate_enhanced_malefic_penalty(
                            chart, planet, target_planet, aspect,
                            aspect_timing, earliest_perfection_days,
